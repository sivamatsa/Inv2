...
(function () {
  ...
  async function renderScheduleTab(container, deals) {
    ...
    const schedule = await App.api.listSchedule({ in: { status: ['UPCOMING', 'DUE_TODAY', 'OVERDUE', 'PARTIALLY_RECEIVED'] } });
    schedule.sort((a, b) => a.scheduled_date.localeCompare(b.scheduled_date));
    container.innerHTML = `
      <div class="table-scroll"><table class="data">
        <thead><tr><th>Scheduled Date</th><th>Deal</th><th>Expected Interest</th><th>Expected Principal</th><th>Expected Total</th><th>Status</th><th>Actions</th></tr></thead>
        <tbody>${schedule.map((s) => `
          ...
          </tr>`).join('') || '<tr><td colspan="7" style="text-align:center;color:var(--text3);padding:24px">Nothing pending.</td></tr>'}</tbody>
      </table></div>`;
    App.utils.qsa('[data-record]', container).forEach((b) => b.addEventListener('click', () => {
      const s = schedule.find((x) => x.id === Number(b.dataset.record));
  ...
  async function renderLedgerTab(container, deals) {
    const dealsById = {}; deals.forEach((d) => { dealsById[d.id] = d; });
    const payments = await App.api.listPayments();
    container.innerHTML = `
      <div style="display:flex;justify-content:flex-end;margin-bottom:10px">
        <button class="btn btn-gold btn-sm" id="adhocRecordBtn">+ Record Payment</button>
      </div>
      <div class="table-scroll"><table class="data">
        <thead><tr><th>Date</th><th>Deal</th><th>Amount</th><th>Interest</th><th>Principal</th><th>Reference</th><th>Method</th><th></th><th>Actions</th></tr></thead>
        <tbody>${payments.map((p) => `
          ...
          </tr>`).join('') || '<tr><td colspan="9" style="text-align:center;color:var(--text3);padding:24px">No payments recorded yet.</td></tr>'}</tbody>
      </table></div>`;
    App.utils.qs('#adhocRecordBtn', container).addEventListener('click', () => openRecordPaymentModal(deals, null, null));
    App.utils.qsa('[data-void]', container).forEach((b) => b.addEventListener('click', async () => {
  ...
  async function renderReconciliationTab(container, deals) {
    const dealsById = {}; deals.forEach((d) => { dealsById[d.id] = d; });
    const [bankTx, schedule] = await Promise.all([
      App.api.listBankTransactions(), App.api.listSchedule({ in: { status: ['UPCOMING', 'DUE_TODAY', 'OVERDUE', 'PARTIALLY_RECEIVED'] } }),
    ]);
    ...
    container.innerHTML = `
      ...
      <div class="table-scroll" style="margin-top:16px"><table class="data">
        ...
        <tbody>${bankTx.filter((t) => !t.matched).map((t) => {
          const s = suggestMatch(t);
          const dealName = s ? (dealsById[s.deal_id] || {}).deal_name : null;
          return `<tr>
            <td>${App.utils.fmtDate(t.transaction_date)}</td>
            <td>${App.utils.fmtMoney(t.amount)}</td>
            <td>${App.utils.escapeHtml(t.description || '—')}</td>
            <td>${s ? `${App.utils.escapeHtml(dealName)} · expected ${App.utils.fmtMoney(s.expected_total)} on ${App.utils.fmtDate(s.scheduled_date)}` : 'No confident match'}</td>
            <td class="row-actions">
              ${s ? `<button class="btn btn-sm btn-teal" data-confirm="${t.id}" data-sched="${s.id}" data-deal="${s.deal_id}">Confirm</button>` : ''}
    ...
    App.utils.qsa('[data-confirm]', container).forEach((b) => b.addEventListener('click', async () => {
      const txId = Number(b.dataset.confirm), schedId = Number(b.dataset.sched), dealId = Number(b.dataset.deal);
      ...
      try {
        await App.api.recordPayment({
          ...
          paymentReference: tx.reference, confirmationMethod: 'Bank Statement', scheduledPaymentId: schedId,
        });
        ...
        App.utils.toast('Match confirmed and payment recorded');
        App.router.refreshCurrent();
  ...
  async function renderPaymentsView() {
    ...
    pane.innerHTML = `
      ...
      <div class="panel">
        <div class="tabbar">
          <button class="tab-btn active" data-tab="sched">Payment Schedule</button>
          <button class="tab-btn" data-tab="ledger">Receipt Ledger</button>
          <button class="tab-btn" data-tab="recon">Bank Reconciliation</button>
        </div>
        <div class="tab-pane active" data-pane="sched" id="scheduleTabBody"></div>
        <div class="tab-pane" data-pane="ledger" id="ledgerTabBody"></div>
        <div class="tab-pane" data-pane="recon" id="reconTabBody"></div>
      </div>`;
  ...
  App.paymentsView = { openRecordPaymentModal };
})();
